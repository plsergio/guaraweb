...
$.fn.myig = function(g, h, j) {
    ...
    // $(this).html('<div class="myig_gallery"></div>');
    // myig_profile(k);
    myig_gallery(k, "");
    ...
    function myig_gallery(e, f) {
        $.ajax({
            ...
            $.each(c.data, function(i, a) {
                ...
                // d += '	</a>';
                // d += '</div>'
                // d += ' <a data-toggle="modal" data-target="#clickModal" href="' + c.data[i].images.standard_resolution.url.replace(/\\/, "") + '" class="myig_popup" rel="myig_popup" title="' + b + '">';
                // d += '	<a href="' + c.data[i].images.standard_resolution.url.replace(/\\/, "") + '" class="myig_popup" rel="myig_popup" title="' + b + '">';
                // d += '   <div class="item"><img class="lazyOwl" src="' + c.data[i].images.thumbnail.url.replace(/\\/, "") + '" alt="Evento1">';
                // d += '   </div>';
                d += '<img src="https://www.instagram.com/p/BI_fM-ZACPk/media/?size=l">';
                // d += ' </a>';
            });
